...
- Внутри должен быть блок Mermaid:
  ...
  ```
  ```
- На главной странице отображается **только диаграмма** (без заголовков/подписей)
- Для иконок в `flowchart` используем iconify-названия (Mermaid v11+), например: `fa6-solid:user`
